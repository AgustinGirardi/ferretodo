...
export default function CartPage() {
    ...
    return (
      <div className="container mx-auto flex flex-col items-center gap-4 px-4 py-20 text-center">
        ...
        <p className="max-w-sm text-sm text-muted">
          Agregá productos y aparecerán acá. ¿No sabés por dónde empezar?
        </p>
        <Link href="/productos">
  ...
  const suggestions = bestSellers.filter((p) => !items.some((i) => i.id === p.id)).slice(0, 4);
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1fr_340px]">
        <div>
          {items.map((i) => (
            <CartItemRow key={i.id} productId={i.id} qty={i.qty} />
          ))}
          <Link
            href="/productos"
